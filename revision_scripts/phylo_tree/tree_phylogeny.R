...
create_viridis_palette <- function(data) {
  ...
  # Generate a viridis color palette, ordering species by their distance values
...
# Convert `distance_data` into a named numeric vector
...
# ✅ Prune the tree to include only species from `distance_data`
tree_scenario1 <- keep.tip(tree_scenario1, distance_data$species)

# Assign colors to species in `distance_data`
