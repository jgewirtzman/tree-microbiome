...
# Function to safely perform t-test with error handling
safe_t_test <- function(formula, data, test_name) {
  tryCatch({
    # Check if data has sufficient observations
    if(nrow(data) < 2) {
      return(paste("Error in", test_name, ": Insufficient data"))
    }
    
    # Check if data has variation
    groups <- split(data, data[[all.vars(formula)[2]]])
    if(any(sapply(groups, function(x) length(unique(x[[all.vars(formula)[1]]]))) <= 1)) {
      return(paste("Error in", test_name, ": Data essentially constant"))
    }
    
    t.test(formula, data = data, var.equal = FALSE)
  }, error = function(e) {
    paste("Error in", test_name, ":", e$message)
  })
}

# O2 concentration
t_test_O2 <- safe_t_test(O2_Area ~ Tissue, df_filtered, "O2 test")

# CH4 concentration (log-transformed)
t_test_CH4 <- safe_t_test(log10ch4 ~ Tissue, df_filtered, "CH4 test")

# pmoA abundance (log-transformed) - Remove NAs before the test
pmoa_data <- ddpcr %>% filter(!is.na(pmoa_loose) & pmoa_loose > 0 & !is.infinite(log10(pmoa_loose)))
t_test_pmoA <- safe_t_test(log10(pmoa_loose) ~ core_type, pmoa_data, "pmoA test")

# mcrA abundance (log-transformed) - Remove NAs before the test
mcra_data <- ddpcr %>% filter(!is.na(mcra_probe_loose) & mcra_probe_loose > 0 & !is.infinite(log10(mcra_probe_loose)))
t_test_mcrA <- safe_t_test(log10(mcra_probe_loose) ~ core_type, mcra_data, "mcrA test")

# Aerobic chemoheterotrophy - Remove NAs before the test
aerobic_data <- faprotax %>% filter(!is.na(aerobic_chemoheterotrophy) & !is.infinite(aerobic_chemoheterotrophy))
t_test_aerobic_chemo <- safe_t_test(aerobic_chemoheterotrophy ~ core_type, aerobic_data, "aerobic chemo test")

# Fermentation - Remove NAs before the test
ferment_data <- faprotax %>% filter(!is.na(fermentation) & !is.infinite(fermentation))
t_test_fermentation <- safe_t_test(fermentation ~ core_type, ferment_data, "fermentation test")

# Hydrogenotrophic methanogenesis (log-transformed) - Remove NAs and ensure values > 0
hydrogen_data <- faprotax %>% filter(!is.na(hydrogenotrophic_methanogenesis) & hydrogenotrophic_methanogenesis > 0 & !is.infinite(log10(hydrogenotrophic_methanogenesis)))
t_test_hydrogen_methano <- safe_t_test(log10(hydrogenotrophic_methanogenesis) ~ core_type, hydrogen_data, "hydrogen methanogenesis test")

# Methanotrophy (log-transformed) - Remove NAs and ensure values > 0
methano_data <- faprotax %>% filter(!is.na(methanotrophy) & methanotrophy > 0 & !is.infinite(log10(methanotrophy)))
t_test_methanotrophy <- safe_t_test(log10(methanotrophy) ~ core_type, methano_data, "methanotrophy test")
