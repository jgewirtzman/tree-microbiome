...
prepare_Genus_level_data <- function(otu_table_path) {
  ...
  # Remove samples with less than 4000 reads
  sample_sums <- colSums(abundance_matrix)
  low_depth_samples <- names(sample_sums[sample_sums < 4000])
  
  if(length(low_depth_samples) > 0) {
    message(paste("Removing samples with < 4000 reads:", 
  ...
  rarefied_matrix <- t(rrarefy(t(abundance_matrix), sample = 4000))
  
  # Aggregate by Genus
  Genus_aggregated <- rarefied_matrix %>%
    data.frame() %>%
    mutate(Genus = raw_data$Genus) %>%
    group_by(Genus) %>%
...
prepare_relative_abundance <- function(Genus_data) {
  Genus_data %>%
...
processed_data <- prepare_Genus_level_data("Black Oak/ITS/OTU_table.txt")
Genus_level_data <- prepare_relative_abundance(processed_data$data)
low_depth_samples <- processed_data$removed_samples

# Get top 25 Genuses by total relative abundance across all components
top_25_Genuses <- Genus_level_data %>%
  group_by(Genus) %>%
  summarize(
...
plot_data <- Genus_level_data %>%
  filter(Genus %in% top_25_Genuses) %>%
  mutate(
    Genus = factor(Genus, levels = rev(top_25_Genuses)),
    Media = factor(Media, levels = c("Heartwood", "Sapwood", "Bark", "Branch", 
                                     "Foliage", "Leaf Litter", "Coarse Root", 
                                     "Fine Root", "Rot", "Organic Soil", 
                                     "Mineral Soil"))
  ) %>%
  complete(Genus, Media, fill = list(RelativeAbundance = 0))

# ------------------------------
# Step 4: Create Plot
# ------------------------------

p <- ggplot(plot_data, aes(x = Media, y = Genus, fill = RelativeAbundance)) +
...
print("\nTop 25 Genuses by total abundance:")
print(top_25_Genuses)
